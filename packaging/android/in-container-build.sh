...
# Inject custom gradle properties so that androiddeployqt's internal gradle
# invocation picks them up.  gradle.properties is generated by androiddeployqt
# on the first build; on incremental builds it already exists.
GRADLE_PROPS_FILE="${BUILDROOT}/build-android/android-build/gradle.properties"
if [ -f "${GRADLE_PROPS_FILE}" ]; then
	sed -i '/^subsurfaceDebuggable=/d' "${GRADLE_PROPS_FILE}"
	sed -i '/^subsurfaceApplicationId=/d' "${GRADLE_PROPS_FILE}"
	if [ "${APK_BUILD_TYPE}" = "debug" ]; then
		echo "subsurfaceDebuggable=true" >> "${GRADLE_PROPS_FILE}"
	fi
	if [ -n "${APP_ID}" ]; then
		echo "subsurfaceApplicationId=${APP_ID}" >> "${GRADLE_PROPS_FILE}"
	fi
fi

# Delete the APK so cmake always re-runs androiddeployqt (it won't notice
# changes to gradle.properties on its own).
rm -f "${BUILDROOT}/build-android/android-build/subsurface-mobile.apk"

cmake --build .

# On a fresh build, gradle.properties was just created by androiddeployqt
# but without our custom properties.  Inject them and rebuild.
if [ -f "${GRADLE_PROPS_FILE}" ]; then
	NEED_REBUILD=0
	if [ "${APK_BUILD_TYPE}" = "debug" ] && ! grep -q '^subsurfaceDebuggable=true$' "${GRADLE_PROPS_FILE}"; then
		echo "subsurfaceDebuggable=true" >> "${GRADLE_PROPS_FILE}"
		NEED_REBUILD=1
	fi
	if [ -n "${APP_ID}" ] && ! grep -q "^subsurfaceApplicationId=${APP_ID}$" "${GRADLE_PROPS_FILE}"; then
		echo "subsurfaceApplicationId=${APP_ID}" >> "${GRADLE_PROPS_FILE}"
		NEED_REBUILD=1
	fi
	if [ "${NEED_REBUILD}" = "1" ]; then
		echo "=== Rebuilding APK with updated gradle properties ==="
		rm -f "${BUILDROOT}/build-android/android-build/subsurface-mobile.apk"
		cmake --build .
	fi
fi

# Optionally build an Android App Bundle (no androiddeployqt equivalent).
...
if [ "${BUILD_AAB}" = "1" ]; then
	...
	if [ "${APK_BUILD_TYPE}" = "debug" ]; then
		GRADLE_PROPS="${GRADLE_PROPS} -PsubsurfaceDebuggable=true"
	fi
	echo "=== Building AAB ==="
	./gradlew bundleRelease ${GRADLE_PROPS}
fi

# --- Collect artifacts, sign, and fix ownership ---
...
if [ "${BUILD_AAB}" = "1" ]; then
	...
	if [ -z "${AAB}" ]; then
		echo "Error: Unable to locate AAB"
		exit 1
	fi
	cp "${AAB}" "${OUTPUT_DIR}/Subsurface-mobile-${VERSION}.aab"
fi

# Sign with the release keystore if credentials were provided
if [ -n "${KS_ALIAS}" ]; then
	echo "=== Signing ==="
	BT="${ANDROID_SDK_ROOT}/build-tools/${SDK_VERSION}"

	"${BT}/zipalign" -f -P 16 4 "${OUTPUT_APK}" /tmp/aligned.apk
	"${BT}/apksigner" sign \
		--ks /tmp/keystore \
	...
	if [ "${BUILD_AAB}" = "1" ]; then
		jarsigner -sigalg SHA256withRSA -digestalg SHA-256 \
			...
			-storepass "${KS_PASS}" \
			"${OUTPUT_DIR}/Subsurface-mobile-${VERSION}.aab" \
			"${KS_ALIAS}"
	fi
fi

# Fix ownership so the host user can access the output files
